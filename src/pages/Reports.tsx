...
const MOCK_TRANSACTIONS: TransactionRecord[] = [
    {
        ...
        change_amount: 4000,
        items: [
            { product_id: '1', product_name: 'Indomie Goreng', qty: 5, unit: 'Pcs', unit_price: 3500, subtotal: 17500 },
            { product_id: '2', product_name: 'Teh Pucuk Harum 350ml', qty: 10, unit: 'Botol', unit_price: 4000, subtotal: 40000 },
            { product_id: '3', product_name: 'Gudang Garam Filter 12', qty: 2, unit: 'Bungkus', unit_price: 25000, subtotal: 50000 },
            { product_id: '4', product_name: 'Beras Raja Lele 5kg', qty: 1, unit: 'Karung', unit_price: 75000, subtotal: 75000 },
        ]
    },
    {
        id: 'TX-002',
        ...
        change_amount: 0,
        items: [
            { product_id: '2', product_name: 'Teh Pucuk Harum 350ml', qty: 6, unit: 'Botol', unit_price: 4000, subtotal: 24000 },
            { product_id: '5', product_name: 'Aqua 600ml', qty: 3, unit: 'Botol', unit_price: 6000, subtotal: 18000 },
        ]
    },
    {
        id: 'TX-003',
        ...
        items: [
            { product_id: '3', product_name: 'Gudang Garam Filter 12', qty: 20, unit: 'Bungkus', unit_price: 25000, subtotal: 500000 },
            { product_id: '6', product_name: 'Rokok Sampoerna', qty: 10, unit: 'Bungkus', unit_price: 35000, subtotal: 350000 },
        ]
    },
    {
        id: 'TX-004',
...
export default function Reports() {
    ...
    const getMethodBadge = (method: string) => {
        switch (method) {
            case 'CASH': return <span className="bg-green-500/10 text-green-400 px-2.5 py-1 rounded-full text-xs font-bold flex items-center gap-1 border border-green-500/20"><Banknote size={12} /> Cash</span>;
            case 'NON_CASH': return <span className="bg-blue-500/10 text-blue-400 px-2.5 py-1 rounded-full text-xs font-bold flex items-center gap-1 border border-blue-500/20"><Smartphone size={12} /> Non-Cash</span>;
            case 'KREDIT': return <span className="bg-red-500/10 text-red-400 px-2.5 py-1 rounded-full text-xs font-bold flex items-center gap-1 border border-red-500/20"><CreditCard size={12} /> Kredit</span>;
            default: return null;
        }
    };
                    ...
                    {MOCK_TRANSACTIONS.map(tx => (
                        <div key={tx.id} className="bg-[#0f172a] p-4 rounded-xl shadow-sm border border-slate-800 hover:border-primary/40 hover:shadow-lg transition-all group">
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <div className="text-xs font-medium text-slate-500 uppercase tracking-wider">{tx.id}</div>
                                    <div className="text-sm text-slate-400 flex items-center gap-1 mt-0.5">
                                        {new Date(tx.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} • {tx.items_count} item
                                    </div>
                                </div>
                            ...
                            <div className="flex justify-between items-end">
                                <div>
                                    <div className="text-lg font-black text-white">{formatCurrency(tx.total)}</div>
                                    {tx.customer_name && (
                    ...
                    <div className="bg-[#0f172a] w-full sm:max-w-md sm:mx-auto rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90vh] text-white animate-in slide-in-from-bottom duration-300">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#020617]">
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    <div className="text-xs font-medium text-slate-500 uppercase tracking-wider">{selectedTx.id}</div>
                                    <div className="text-sm text-slate-400 flex items-center gap-1 mt-0.5">
                                        <Clock size={14} />
                            ...
                            {selectedTx.customer_name && (
                                <div className="bg-[#0f172a] border border-slate-800 p-4 rounded-xl">
                                    ...
                                    <div className="text-white font-bold">{selectedTx.customer_name}</div>
                            ...
                            {selectedTx.cashier_name && (
                                <div className="flex items-center gap-2 text-slate-500 text-xs">
                                    <User size={14} />
                                    <span>Kasir: <span className="text-slate-300">{selectedTx.cashier_name}</span></span>
                                </div>
                            )}
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center gap-2 text-primary font-black uppercase tracking-widest text-xs">
                                    <Package size={16} />
                                    Item Pembelian ({selectedTx.items?.length || selectedTx.items_count})
                                </div>
                                <div className="space-y-2">
                                    {selectedTx.items?.map((item, idx) => (
                                        <div key={idx} className="bg-[#0f172a] border border-slate-800 p-3 rounded-xl flex justify-between items-center">
                            ...
                            <div className="bg-[#0f172a] border border-slate-800 p-4 rounded-xl space-y-2">
                                <div className="flex justify-between text-slate-400 text-sm">
                                    <span>Subtotal</span>
                                    <span className="text-white">{formatCurrency(selectedTx.total)}</span>
                                </div>
                                {selectedTx.payment_method === 'CASH' && selectedTx.paid_amount !== undefined && (
                                    <>
                                        <div className="flex justify-between text-slate-400 text-sm">
                                            <span>Bayar</span>
                                            <span className="text-white">{formatCurrency(selectedTx.paid_amount)}</span>
                                        </div>
                                        {selectedTx.change_amount !== undefined && selectedTx.change_amount > 0 && (
                                            <div className="flex justify-between text-slate-400 text-sm">
                                                <span>Kembalian</span>
                                                <span className="text-green-400">{formatCurrency(selectedTx.change_amount)}</span>
                                            </div>
                                        )}
                                    </>
                                )}
                                <div className="border-t border-slate-700 pt-2 mt-2">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-2xl font-black text-primary">{formatCurrency(selectedTx.total)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
