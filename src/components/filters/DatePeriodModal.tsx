                ...
                <div className="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {PERIOD_OPTIONS.map((option) => (
                                <button
                                    key={option.value}
                                    onClick={() => option.value === 'custom' ? handlePeriodChange(option.value) : handleQuickSelect(option.value)}
                                    className={`
                                ...
                                <Popover open={isStartPickerOpen} onOpenChange={setIsStartPickerOpen}>
                                    <PopoverTrigger asChild>
                                        ...
                                        >
                                            ...
                                            {startDate ? format(startDate, "PPP", { locale: id }) : <span className="text-slate-500">Pilih tanggal</span>}
                                        </button>
                                    </PopoverTrigger>
                                    <PopoverContent
                                    ...
                                    >
                                        <Calendar
                                            ...
                                            initialFocus
                                            locale={id}
                                            className="bg-[#1a1f2e] text-white [&_.rdp-head_cell]:w-10 [&_.rdp-day]:w-10 [&_.rdp-day]:h-10"
                                            classNames={{
                                                day: "text-white hover:bg-slate-700 rounded-lg text-sm",
                                                day_selected: "bg-primary text-white hover:bg-primary/90 font-bold",
                                                ...
                                                weekday: "text-slate-400 text-xs font-medium py-2",
                                                head_cell: "w-10",
                                            }}
                                            formatters={{
                                                formatWeekdayName: (date) => format(date, 'EEE', { locale: id }).toUpperCase(),
                                            }}
                                        />
                                    </PopoverContent>
                                </Popover>
                            ...
                            <div className="space-y-1">
                                ...
                                <Popover open={isEndPickerOpen} onOpenChange={setIsEndPickerOpen}>
                                    <PopoverTrigger asChild>
                                        ...
                                        >
                                            <CalendarIcon size={18} className="text-primary flex-shrink-0" />
                                            {endDate ? format(endDate, "PPP", { locale: id }) : <span className="text-slate-500">Pilih tanggal</span>}
                                        </button>
                                    </PopoverTrigger>
                                    <PopoverContent
                                        className="w-auto p-0 border-slate-700 bg-[#1a1f2e] z-[200]"
                    ...
                    {selectedPeriod !== 'custom' && (
                        <div className="bg-slate-800/50 rounded-xl p-4 text-center">
                            <div className="text-xs text-slate-500 mb-1">Periode yang dipilih</div>
                            <div className="text-lg font-bold text-primary">{getPeriodLabel(selectedPeriod)}</div>
                        </div>
                    )}
                </div>
